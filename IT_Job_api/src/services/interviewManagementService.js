...
const saveInterview = async (dialogType, formData, job, applicant, roundId, interview = null) => {
    try {
        if (dialogType === 'add') {
            ...
            // Update applicant progress
            console.log("heheheheheheh",dialogType, formData, job, applicant, roundId);
            await updateApplicantProgress(
                applicant.id || applicant._id,
                job.id || job._id,
                interview.id,
            ...
            if (formData.evaluation.result === 'Đạt') {
                ...
                //     applicantEmail,
                //     'Thông báo kết quả phỏng vấn',
                //     emailTemplates.interviewRejection(applicantName, jobTitle)
                // )
            }
            return updatedInterview
...
const sendInvitation = async (interviewId) => {
    try {
        ...
        // Send email
        console.log(application.email, location);
        await BrevoProvider.sendEmail(
            application.email,
            'Thư mời phỏng vấn',
